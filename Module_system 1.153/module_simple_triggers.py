...
# This trigger will check if player's raid has been completed and will lead control to village menu.
  ...
  (24 * 7,
   [
######### sim_tr ####### TU ######### juewei ##
	(try_for_range,":num",0,50),
		(troop_set_slot, "trp_temp_troop",":num",0),
	(try_end),
	(try_begin),
		...
		(try_begin),
			(ge, ":troop_title", 40),
			(val_sub, ":troop_title", 40),
			(val_add, ":troop_title", 1),#wen guan > wuguan 
		(try_end),
		(store_sub, ":troop_title", 40, ":troop_title"),
		(troop_get_slot, ":div_num", "trp_temp_troop","$players_kingdom"),
		(val_add, ":div_num", ":troop_title"),
		(troop_set_slot, "trp_temp_troop","$players_kingdom", ":div_num"),
	(try_end),
	
	(try_for_range, ":troop_no", kings_begin, pretenders_begin),
		(store_troop_faction, ":faction_no", ":troop_no"),
		(is_between, ":faction_no", kingdoms_begin, kingdoms_end),
		(troop_get_slot, ":troop_title", ":troop_no", slot_troop_title_level),
		(try_begin),
			(ge, ":troop_title", 40),
			(val_sub, ":troop_title", 40),
			(val_add, ":troop_title", 1),#wen guan > wuguan
		(try_end),
		(store_sub, ":troop_title", 40, ":troop_title"),
		(troop_get_slot, ":div_num", "trp_temp_troop",":faction_no"),
		(val_add, ":div_num", ":troop_title"),
		(troop_set_slot, "trp_temp_troop",":faction_no", ":div_num"),
	(try_end),
	
	(assign, "$fysg_gongzi", 0),
	(try_begin),
		(is_between, "$players_kingdom", kingdoms_begin, kingdoms_end),
		(troop_get_slot, ":troop_title", "trp_player", slot_troop_title_level),
		(try_begin),
		...
		(troop_get_slot, ":div_num", "trp_temp_troop","$players_kingdom"),
		(gt, ":div_num", 0),
		(faction_get_slot, ":total_money", "$players_kingdom",slot_gongzi_next_month),
		(gt, ":total_money", 0),
		(val_mul, ":total_money", ":troop_title"),
		(val_div, ":total_money", ":div_num"),
		(val_div, ":total_money", 10),
		(assign, "$fysg_gongzi",":total_money"),
	(try_end),
	(try_for_range, ":troop_no", kings_begin, pretenders_begin),
		(store_troop_faction, ":faction_no", ":troop_no"),
		(is_between, ":faction_no", kingdoms_begin, kingdoms_end),
		...
		(troop_get_slot, ":div_num", "trp_temp_troop",":faction_no"),
		(gt, ":div_num", 0),
		(faction_get_slot, ":total_money", ":faction_no",slot_gongzi_next_month),
		(val_mul, ":total_money", ":troop_title"),
		(val_div, ":total_money", ":div_num"),
		
		(troop_get_slot, ":troop_wealth", ":troop_no", slot_troop_wealth),
		(val_add, ":troop_wealth", ":total_money"),
  ...
  # Process siege ai
   ...
   [
      ...
      (try_for_range, ":center_no", walled_centers_begin, walled_centers_end),
		...
		############# TU ##### neizheng ### 7.20 ##
		(party_get_slot, ":party_defense", ":center_no", slot_party_defense),
		(val_div, ":party_defense", 10),
		(val_sub, ":siege_begin_hours", ":party_defense"),
		############################################
...
############# juewei ####### sim_tr ######## TU ###########
			...
			(assign, ":oid_rents", ":cur_rents"),
			(store_div,":rennt_1_20",":cur_rents",20),
			(faction_get_slot,":money_now",":faction_no", slot_faction_money),
			(store_mul,":rennt_1_20",":rennt_1_20",1),
			(val_add,":money_now",":rennt_1_20"),
			(val_sub,":cur_rents",":rennt_1_20"),
			(faction_set_slot,":faction_no", slot_faction_money,":money_now"),
			(store_mul,":rennt_10_20",":rennt_1_20",10),
			(faction_get_slot, ":gongzi_next_month",":faction_no", slot_gongzi_next_month),
			(val_add, ":gongzi_next_month", ":rennt_10_20"),
			(faction_set_slot,":faction_no", slot_gongzi_next_month,":gongzi_next_month"),
			(val_sub,":cur_rents",":rennt_10_20"),
##############       
  ...
  (48, 
   [
       (try_for_range, ":troop_no", active_npcs_begin, active_npcs_end),
		 ...
		 (neg|main_party_has_troop, ":troop_no"),  #TU

         (store_troop_faction, ":cur_faction", ":troop_no"),
         (try_begin),
           (eq, ":cur_faction", "fac_outlaws"), #Do nothing
########## TU #### zhaoxiang ## simp_trig ## 7.18 #######
         (else_try),  
			(eq, ":cur_faction", "fac_undeads"),
         (else_try),  
			(eq, ":cur_faction", "fac_unborn"),
########################
		 (else_try),  
           (try_begin),
             (eq, "$cheat_mode", 2),
...
########## TU ######### neizheng ##### 7.20 #####
  (24 * 6,
   [
	(try_for_range, ":village_no", villages_begin, villages_end),
		(party_get_slot, ":min_dist_town", ":village_no", slot_village_bound_center),
		(party_get_slot, ":weekly_plan", ":min_dist_town", slot_party_weekly_plan),
		(eq, ":weekly_plan", 0),
		...
		(party_get_slot, ":volunteer_troop", ":village_no", slot_center_volunteer_troop_type),
		
		(party_get_slot, ":prosperity", ":min_dist_town", slot_town_prosperity),
		(store_div, ":agent_most_num", ":prosperity", 10),
		(val_min, ":agent_most_num", 9),
		(val_mul, ":agent_most_num", 30),
		(val_add, ":agent_most_num", 300),
		(party_get_num_companions, ":agent_num", ":min_dist_town"),
		(val_add, ":agent_num", ":volunteer_amount"),
		(try_begin),
			(lt, ":agent_num", ":agent_most_num"),
			(neq, ":volunteer_troop", 0),
			(party_add_members, ":min_dist_town", ":volunteer_troop", ":volunteer_amount"),
		(try_end),
	(try_end),
	(assign, ":player_have_center", 0),
	(try_for_range, ":center_no", walled_centers_begin, walled_centers_end),
		(party_get_slot, ":weekly_plan", ":center_no", slot_party_weekly_plan),
		(party_get_slot, ":center_lord", ":center_no", slot_town_lord),
		(try_begin),
			(gt, ":weekly_plan", 0),
			(ge, ":center_lord", 0),
			(try_begin),
				...
				(val_add, ":trainer_level", 5), #average trainer level is 3 for npc lords, worst : 0, best : 6
				(store_mul, ":xp_gain", ":trainer_level", 200), #xp gain in two days of period for each lord, average : 8000.
				(assign, ":xp_gain", 3000), #xp gain in two days of period for each center, average : 3000.
				(game_get_reduce_campaign_ai, ":reduce_campaign_ai"),
				(try_begin),
					(eq, ":reduce_campaign_ai", 0), #hard (1.5x)
				...
				(party_upgrade_with_xp, ":center_no", ":xp_gain"),
			(else_try),
				(eq, ":weekly_plan", 2),#economic 
				(store_skill_level, ":trade_level", skl_trade, ":center_lord"), 
				(val_add, ":trade_level", 5),
				(val_mul, ":trade_level", 2),
				(val_div, ":trade_level", 3),
				(party_get_slot, ":prosperity", ":center_no", slot_town_prosperity),
				(store_sub, ":prosperity_add", 100, ":prosperity"),
				(gt, ":prosperity_add", 0),
				(val_mul, ":prosperity_add", ":trade_level"),
				(val_div, ":prosperity_add", 80),
				(val_add, ":prosperity_add", ":prosperity"),
				(party_set_slot, ":center_no", slot_town_prosperity, ":prosperity_add"),
			(else_try),
				(eq, ":weekly_plan", 3),#party_defense 
				(store_skill_level, ":engineer_level", skl_engineer, ":center_lord"), 
				(val_add, ":engineer_level", 5),
				(party_get_slot, ":party_defense", ":center_no", slot_party_defense),
				(val_add, ":party_defense", ":engineer_level"),
				(party_set_slot, ":center_no", slot_party_defense, ":party_defense"),
			(try_end),
		(try_end),
		(try_begin),
			(eq, ":center_lord", "trp_player"),
			# (assign, "$player_neizheng_center", ":center_no"),
			(assign, ":player_have_center", 1),
		(else_try),
			(store_random_in_range, ":rand", 0, 4),
			(party_set_slot, ":center_no", slot_party_weekly_plan, ":rand"),
		(try_end),
	(try_end),
	(try_begin),
		(eq, ":player_have_center", 1),
		(start_presentation, "prsnt_neizheng_center"),
	(try_end),
   ]),
###########################
#######JL##########
  # (3,
    # [
      # (try_for_parties, ":party_no"),
        ...
        # (is_between, ":template_id", "pt_looters", "pt_merchant_caravan"),
  ...
  (24,
    [
      (party_get_attached_to, ":cur_center_no", "p_main_party"),
      (is_between, ":cur_center_no", walled_centers_begin, walled_centers_end),
      (party_slot_eq, ":cur_center_no", slot_town_lord, "trp_player"),
	  ####wei#####
	  (party_slot_eq, ":cur_center_no", slot_town_lord, "trp_kingdom_1_lord"),
	  (party_slot_eq, ":cur_center_no", slot_town_lord, "trp_kingdom_2_lord"),
	  (party_slot_eq, ":cur_center_no", slot_town_lord, "trp_kingdom_3_lord"),
	  (party_slot_eq, ":cur_center_no", slot_town_lord, "trp_kingdom_4_lord"),
	  (party_slot_eq, ":cur_center_no", slot_town_lord, "trp_kingdom_5_lord"),
	  (party_slot_eq, ":cur_center_no", slot_town_lord, "trp_kingdom_6_lord"),
	  (party_slot_eq, ":cur_center_no", slot_town_lord, "trp_knight_1_1"),
	  (party_slot_eq, ":cur_center_no", slot_town_lord, "trp_knight_1_2"),
	  (party_slot_eq, ":cur_center_no", slot_town_lord, "trp_knight_1_3"),
	  (party_slot_eq, ":cur_center_no", slot_town_lord, "trp_knight_1_4"),
	  (party_slot_eq, ":cur_center_no", slot_town_lord, "trp_knight_1_5"),
	  (party_slot_eq, ":cur_center_no", slot_town_lord, "trp_knight_1_6"),
	  (party_slot_eq, ":cur_center_no", slot_town_lord, "trp_knight_1_7"),
	  (party_slot_eq, ":cur_center_no", slot_town_lord, "trp_knight_1_8"),
	  (party_slot_eq, ":cur_center_no", slot_town_lord, "trp_knight_1_9"),
	  (party_slot_eq, ":cur_center_no", slot_town_lord, "trp_knight_1_10"),
	  (party_slot_eq, ":cur_center_no", slot_town_lord, "trp_knight_1_11"),
	  (party_slot_eq, ":cur_center_no", slot_town_lord, "trp_knight_1_12"),
	  (party_slot_eq, ":cur_center_no", slot_town_lord, "trp_knight_1_13"),
	  (party_slot_eq, ":cur_center_no", slot_town_lord, "trp_knight_1_14"),
	  (party_slot_eq, ":cur_center_no", slot_town_lord, "trp_knight_1_15"),
	  (party_slot_eq, ":cur_center_no", slot_town_lord, "trp_knight_1_16"),
	  (party_slot_eq, ":cur_center_no", slot_town_lord, "trp_knight_1_17"),
	  (party_slot_eq, ":cur_center_no", slot_town_lord, "trp_knight_1_18"),
	  (party_slot_eq, ":cur_center_no", slot_town_lord, "trp_knight_1_19"),
	  (party_slot_eq, ":cur_center_no", slot_town_lord, "trp_knight_1_20"),
	  #####shu####
	  (party_slot_eq, ":cur_center_no", slot_town_lord, "trp_knight_2_1"),
	  (party_slot_eq, ":cur_center_no", slot_town_lord, "trp_knight_2_2"),
	  (party_slot_eq, ":cur_center_no", slot_town_lord, "trp_knight_2_3"),
	  (party_slot_eq, ":cur_center_no", slot_town_lord, "trp_knight_2_4"),
	  (party_slot_eq, ":cur_center_no", slot_town_lord, "trp_knight_2_5"),
	  (party_slot_eq, ":cur_center_no", slot_town_lord, "trp_knight_2_6"),
	  (party_slot_eq, ":cur_center_no", slot_town_lord, "trp_knight_2_7"),
	  (party_slot_eq, ":cur_center_no", slot_town_lord, "trp_knight_2_8"),
	  (party_slot_eq, ":cur_center_no", slot_town_lord, "trp_knight_2_9"),
	  (party_slot_eq, ":cur_center_no", slot_town_lord, "trp_knight_2_10"),
	  (party_slot_eq, ":cur_center_no", slot_town_lord, "trp_knight_2_11"),
	  (party_slot_eq, ":cur_center_no", slot_town_lord, "trp_knight_2_12"),
	  (party_slot_eq, ":cur_center_no", slot_town_lord, "trp_knight_2_13"),
	  (party_slot_eq, ":cur_center_no", slot_town_lord, "trp_knight_2_14"),
	  (party_slot_eq, ":cur_center_no", slot_town_lord, "trp_knight_2_15"),
	  (party_slot_eq, ":cur_center_no", slot_town_lord, "trp_knight_2_16"),
	  (party_slot_eq, ":cur_center_no", slot_town_lord, "trp_knight_2_17"),
	  (party_slot_eq, ":cur_center_no", slot_town_lord, "trp_knight_2_18"),
	  (party_slot_eq, ":cur_center_no", slot_town_lord, "trp_knight_2_19"),
	  (party_slot_eq, ":cur_center_no", slot_town_lord, "trp_knight_2_20"),
	  #####hebei####
	  (party_slot_eq, ":cur_center_no", slot_town_lord, "trp_knight_3_1"),
	  (party_slot_eq, ":cur_center_no", slot_town_lord, "trp_knight_3_2"),
	  (party_slot_eq, ":cur_center_no", slot_town_lord, "trp_knight_3_3"),
	  (party_slot_eq, ":cur_center_no", slot_town_lord, "trp_knight_3_4"),
	  (party_slot_eq, ":cur_center_no", slot_town_lord, "trp_knight_3_5"),
	  (party_slot_eq, ":cur_center_no", slot_town_lord, "trp_knight_3_6"),
	  (party_slot_eq, ":cur_center_no", slot_town_lord, "trp_knight_3_7"),
	  (party_slot_eq, ":cur_center_no", slot_town_lord, "trp_knight_3_8"),
	  (party_slot_eq, ":cur_center_no", slot_town_lord, "trp_knight_3_9"),
	  (party_slot_eq, ":cur_center_no", slot_town_lord, "trp_knight_3_10"),
	  (party_slot_eq, ":cur_center_no", slot_town_lord, "trp_knight_3_11"),
	  (party_slot_eq, ":cur_center_no", slot_town_lord, "trp_knight_3_12"),
	  (party_slot_eq, ":cur_center_no", slot_town_lord, "trp_knight_3_13"),
	  (party_slot_eq, ":cur_center_no", slot_town_lord, "trp_knight_3_14"),
	  (party_slot_eq, ":cur_center_no", slot_town_lord, "trp_knight_3_15"),
	  (party_slot_eq, ":cur_center_no", slot_town_lord, "trp_knight_3_16"),
	  (party_slot_eq, ":cur_center_no", slot_town_lord, "trp_knight_3_17"),
	  (party_slot_eq, ":cur_center_no", slot_town_lord, "trp_knight_3_18"),
	  (party_slot_eq, ":cur_center_no", slot_town_lord, "trp_knight_3_19"),
	  (party_slot_eq, ":cur_center_no", slot_town_lord, "trp_knight_3_20"),
	  ####JINXIANG####
	  (party_slot_eq, ":cur_center_no", slot_town_lord, "trp_knight_4_1"),
	  (party_slot_eq, ":cur_center_no", slot_town_lord, "trp_knight_4_2"),
	  (party_slot_eq, ":cur_center_no", slot_town_lord, "trp_knight_4_3"),
	  (party_slot_eq, ":cur_center_no", slot_town_lord, "trp_knight_4_4"),
	  (party_slot_eq, ":cur_center_no", slot_town_lord, "trp_knight_4_5"),
	  (party_slot_eq, ":cur_center_no", slot_town_lord, "trp_knight_4_6"),
	  (party_slot_eq, ":cur_center_no", slot_town_lord, "trp_knight_4_7"),
	  (party_slot_eq, ":cur_center_no", slot_town_lord, "trp_knight_4_8"),
	  (party_slot_eq, ":cur_center_no", slot_town_lord, "trp_knight_4_9"),
	  (party_slot_eq, ":cur_center_no", slot_town_lord, "trp_knight_4_10"),
	  (party_slot_eq, ":cur_center_no", slot_town_lord, "trp_knight_4_11"),
	  (party_slot_eq, ":cur_center_no", slot_town_lord, "trp_knight_4_12"),
	  (party_slot_eq, ":cur_center_no", slot_town_lord, "trp_knight_4_13"),
	  (party_slot_eq, ":cur_center_no", slot_town_lord, "trp_knight_4_14"),
	  (party_slot_eq, ":cur_center_no", slot_town_lord, "trp_knight_4_15"),
	  (party_slot_eq, ":cur_center_no", slot_town_lord, "trp_knight_4_16"),
	  (party_slot_eq, ":cur_center_no", slot_town_lord, "trp_knight_4_17"),
	  (party_slot_eq, ":cur_center_no", slot_town_lord, "trp_knight_4_18"),
	  (party_slot_eq, ":cur_center_no", slot_town_lord, "trp_knight_4_19"),
	  (party_slot_eq, ":cur_center_no", slot_town_lord, "trp_knight_4_20"),
	  ####wu####
	  (party_slot_eq, ":cur_center_no", slot_town_lord, "trp_knight_5_1"),
	  (party_slot_eq, ":cur_center_no", slot_town_lord, "trp_knight_5_2"),
	  (party_slot_eq, ":cur_center_no", slot_town_lord, "trp_knight_5_3"),
	  (party_slot_eq, ":cur_center_no", slot_town_lord, "trp_knight_5_4"),
	  (party_slot_eq, ":cur_center_no", slot_town_lord, "trp_knight_5_5"),
	  (party_slot_eq, ":cur_center_no", slot_town_lord, "trp_knight_5_6"),
	  (party_slot_eq, ":cur_center_no", slot_town_lord, "trp_knight_5_7"),
	  (party_slot_eq, ":cur_center_no", slot_town_lord, "trp_knight_5_8"),
	  (party_slot_eq, ":cur_center_no", slot_town_lord, "trp_knight_5_9"),
	  (party_slot_eq, ":cur_center_no", slot_town_lord, "trp_knight_5_10"),
	  (party_slot_eq, ":cur_center_no", slot_town_lord, "trp_knight_5_11"),
	  (party_slot_eq, ":cur_center_no", slot_town_lord, "trp_knight_5_12"),
	  (party_slot_eq, ":cur_center_no", slot_town_lord, "trp_knight_5_13"),
	  (party_slot_eq, ":cur_center_no", slot_town_lord, "trp_knight_5_14"),
	  (party_slot_eq, ":cur_center_no", slot_town_lord, "trp_knight_5_15"),
	  (party_slot_eq, ":cur_center_no", slot_town_lord, "trp_knight_5_16"),
	  (party_slot_eq, ":cur_center_no", slot_town_lord, "trp_knight_5_17"),
	  (party_slot_eq, ":cur_center_no", slot_town_lord, "trp_knight_5_18"),
	  (party_slot_eq, ":cur_center_no", slot_town_lord, "trp_knight_5_19"),
	  (party_slot_eq, ":cur_center_no", slot_town_lord, "trp_knight_5_20"),
	  ####xiliang####
	  (party_slot_eq, ":cur_center_no", slot_town_lord, "trp_knight_6_1"),
	  (party_slot_eq, ":cur_center_no", slot_town_lord, "trp_knight_6_2"),
	  (party_slot_eq, ":cur_center_no", slot_town_lord, "trp_knight_6_3"),
	  (party_slot_eq, ":cur_center_no", slot_town_lord, "trp_knight_6_4"),
	  (party_slot_eq, ":cur_center_no", slot_town_lord, "trp_knight_6_5"),
	  (party_slot_eq, ":cur_center_no", slot_town_lord, "trp_knight_6_6"),
	  (party_slot_eq, ":cur_center_no", slot_town_lord, "trp_knight_6_7"),
	  (party_slot_eq, ":cur_center_no", slot_town_lord, "trp_knight_6_8"),
	  (party_slot_eq, ":cur_center_no", slot_town_lord, "trp_knight_6_9"),
	  (party_slot_eq, ":cur_center_no", slot_town_lord, "trp_knight_6_10"),
	  (party_slot_eq, ":cur_center_no", slot_town_lord, "trp_knight_6_11"),
	  (party_slot_eq, ":cur_center_no", slot_town_lord, "trp_knight_6_12"),
	  (party_slot_eq, ":cur_center_no", slot_town_lord, "trp_knight_6_13"),
	  (party_slot_eq, ":cur_center_no", slot_town_lord, "trp_knight_6_14"),
	  (party_slot_eq, ":cur_center_no", slot_town_lord, "trp_knight_6_15"),
	  (party_slot_eq, ":cur_center_no", slot_town_lord, "trp_knight_6_16"),
	  (party_slot_eq, ":cur_center_no", slot_town_lord, "trp_knight_6_17"),
	  (party_slot_eq, ":cur_center_no", slot_town_lord, "trp_knight_6_18"),
	  (party_slot_eq, ":cur_center_no", slot_town_lord, "trp_knight_6_19"),
	  (party_slot_eq, ":cur_center_no", slot_town_lord, "trp_knight_6_20"),
      (party_get_num_companion_stacks, ":num_stacks_main_party", "p_main_party"),
      (party_get_num_companion_stacks, ":num_stacks", ":cur_center_no"),
      (gt, ":num_stacks", 0),
      (try_for_range, ":stack_no", 0, ":num_stacks"),
        (party_stack_get_troop_id, ":stack_troop", ":cur_center_no", ":stack_no"),
        (party_stack_get_size, ":stack_size", ":cur_center_no", ":stack_no"),
        (store_character_level, ":troop_level", ":stack_troop"),
        (assign, ":total_gained_xp", 0),
        (try_for_range, ":stack_no_main_party", 0, ":num_stacks_main_party"),
          (party_stack_get_troop_id, ":this_hero", "p_main_party", ":stack_no_main_party"),
          ...
          (val_add, ":total_gained_xp", ":given_xp"),
        (try_end),
        (val_mul, ":total_gained_xp", ":stack_size"),
        (party_add_xp_to_stack, ":cur_center_no", ":stack_no", ":total_gained_xp"),
      (try_end),
      (party_upgrade_with_xp, ":cur_center_no", 1, 0),
    ]), 
#####################################


################## TU ####### module_simple_triggers ###### Decide faction ai flag check ##########
   (24,
	[
	 (eq, "$mod_no", 1),
	 (store_current_day, ":time_now"),
	 (val_sub, ":time_now", "$hulaoguan_time_begin"),
	 (try_begin),
		(lt, ":time_now", 8),
		(ge, ":time_now", 5),
		(assign, "$hulaoguan_phase", 2),
	(else_try),
		(lt, ":time_now", 10),
		(ge, ":time_now", 8),
		(assign, "$hulaoguan_phase", 3),
	 (else_try),
		(lt, ":time_now", 15),
		(ge, ":time_now", 10),
		(assign, "$hulaoguan_phase", 4),
	 (else_try),
		(ge, ":time_now", 15),
		(assign, "$hulaoguan_phase", 5),
	 (try_end),
	]),
   ...
   (5,
	...
	(try_begin),
		...
		(store_troop_faction, ":faction_no",  "trp_knight_6_6"),
		(neq, ":faction_no", "fac_undeads"),
		(store_troop_faction, ":faction_no",  "trp_knight_2_1"),
		(neq, ":faction_no", "fac_undeads"),
		(store_troop_faction, ":faction_no",  "trp_knight_2_2"),
		(neq, ":faction_no", "fac_undeads"),
		(store_troop_faction, ":faction_no",  "trp_knight_mod_28"),
		(neq, ":faction_no", "fac_undeads"),
		# (store_troop_faction, ":faction_no",  "trp_knight_mod_30"),
		# (neq, ":faction_no", "fac_undeads"),
		
		(store_distance_to_party_from_party, ":party_distance", "p_main_party", "p_castle_31"),
		(lt, ":party_distance", 7),
		(troop_get_slot, ":party_no", "trp_kingdom_3_lord", slot_troop_leaded_party),
		(neq, ":party_no", -1),
		(store_distance_to_party_from_party, ":party_distance", "p_castle_31", ":party_no"),
		(lt, ":party_distance", 7),
		# (troop_get_slot, ":party_no", "trp_knight_6_6", slot_troop_leaded_party),
		# (neq, ":party_no", -1),
		# (store_distance_to_party_from_party, ":party_distance", "p_castle_31", ":party_no"),
		# (lt, ":party_distance", 7),
		...
		(store_distance_to_party_from_party, ":party_distance", "p_castle_31", ":party_no"),
		(lt, ":party_distance", 7),
		# (troop_get_slot, ":party_no", "trp_kingdom_2_lord", slot_troop_leaded_party),
		# (neq, ":party_no", -1),
		# (store_distance_to_party_from_party, ":party_distance", "p_castle_31", ":party_no"),
		# (lt, ":party_distance", 7),
		
		(assign, "$zhanguaxiong", 0),
		(jump_to_menu, "mnu_hulaoguan"),
	(else_try),
		(eq, "$hulaoguan_phase", 3),
		(troop_get_slot, ":party_no", "trp_kingdom_3_lord", slot_troop_leaded_party),
		(party_get_slot, ":ai_object", ":party_no", slot_party_ai_object),
		(call_script, "script_begin_assault_on_center", ":ai_object"),
		(assign, "$g_leave_encounter", 1),
	(else_try),
		(eq, "$hulaoguan_phase", 5),
		(call_script, "script_appoint_faction_marshall", "fac_kingdom_1", "trp_kingdom_1_lord"),   
		(call_script, "script_appoint_faction_marshall", "fac_kingdom_2", "trp_kingdom_2_lord"),   
		(call_script, "script_appoint_faction_marshall", "fac_kingdom_3", "trp_kingdom_3_lord"),   
		(call_script, "script_appoint_faction_marshall", "fac_kingdom_5", "trp_kingdom_5_lord"),   
		(call_script, "script_appoint_faction_marshall", "fac_kingdom_7", "trp_knight_mod_1"),   
		(call_script, "script_appoint_faction_marshall", "fac_kingdom_8", "trp_knight_mod_2"),   
		(call_script, "script_appoint_faction_marshall", "fac_kingdom_11", "trp_knight_mod_20"),   
		(call_script, "script_appoint_faction_marshall", "fac_kingdom_12", "trp_knight_mod_21"),  
		
		# (party_set_slot, "p_town_10",   slot_center_ex_faction, "fac_kingdom_3"),
	 (try_end),
	]),
   ...
   (24,#born
	[
	  (eq, "$mod_no", 1),
	  (store_current_day, ":current_year"),
	  (val_div, ":current_year", 72),
	  #(val_div, ":current_year", 354),#TU 7.18
	  (val_add, ":current_year", 190),
	  (try_for_range, ":troop_id", kings_begin, kingdom_ladies_end),
		(store_random_in_range, ":random", 1, 72),
		#(store_random_in_range, ":random", 1, 354),#TU 7.18
		(eq, ":random", 3),
		(store_faction_of_troop, ":faction_id", ":troop_id"),
		(eq, ":faction_id", "fac_unborn"),
		(troop_get_slot, ":troop_adult_year", ":troop_id", slot_troop_adult_year),
		(troop_get_slot, ":troop_born_year", ":troop_id", slot_troop_age),
		(val_add, ":troop_born_year", 15),
		(this_or_next|le, ":troop_adult_year", ":current_year"),
		(le, ":troop_born_year", ":current_year"),

		(troop_set_note_available, ":troop_id", 1),
		(troop_get_slot, ":troop_father", ":troop_id", slot_troop_born_father), 
		(assign, ":continue", 1),
		(try_begin),
			(ge, ":troop_father", 0),
			(store_troop_faction, ":troop_father_faction", ":troop_father"), 
			(neg|is_between, ":troop_father_faction", kingdoms_begin, kingdoms_end),
			(assign, ":continue", 0),
		(try_end),
		(eq, ":continue", 1),
		(troop_get_slot, ":troop_guardian", ":troop_id", slot_troop_born_guardian), 
		(assign, ":continue", 1),
		(try_begin),
			(ge, ":troop_guardian", 0),
			(store_troop_faction, ":troop_guardian_faction", ":troop_guardian"), 
			(neg|is_between, ":troop_guardian_faction", kingdoms_begin, kingdoms_end),
			(assign, ":continue", 0),
		(try_end),
		(eq, ":continue", 1),
		(troop_get_slot, ":troop_faction", ":troop_id", slot_troop_born_faction), 
		(troop_set_slot, ":troop_id", slot_troop_father, ":troop_father"), 
		(troop_set_slot, ":troop_id", slot_troop_guardian, ":troop_guardian"), 
		(troop_set_faction, ":troop_id", ":troop_faction"),
		
		(str_store_faction_name, s2, ":troop_faction"),
		(str_store_troop_name, s1, ":troop_id"),
		(dialog_box, "str_somebody_born"),
		(display_message, "str_somebody_born"),
	  (try_end),
	]),
###########################################
...
############ TU ###############
    (0,
     [
         (assign,reg0,0),
         (assign,":times",9),
         (try_for_range, ":slot_no", 0, ":times"),
             (troop_get_slot,":root_troop_id","trp_slot1",":slot_no"),
             (try_begin),
                 (call_script,"script_party_has_troop","p_main_party",":root_troop_id"),
                 (eq,reg0,0),  
                 (assign,reg12,":root_troop_id"),
                 (assign,reg11,":slot_no"),
                 (assign,":times",0),
             (try_end),
         (try_end),
         (eq,reg0,0),
         (store_add,":new_troop_id",reg11,"trp_new_troop0"),
         #(assign,reg10,":new_troop_id"),
         ...
         (try_for_range, ":cur_stack", 0, ":num_stack"),
             ...
             (eq,":cur_troop",":new_troop_id"),
             (party_stack_get_size, ":stack_size", "p_main_party", ":cur_stack"),
             (party_remove_members, "p_main_party", ":new_troop_id", ":stack_size"),
             (party_add_members, "p_main_party", ":root_troop_id", ":stack_size"),
             (troop_set_slot,"trp_slot1",reg11,0),
             (assign,":num_stack",0),
         (try_end),
     ]),
    (0,
     [
         ...
         (assign,":times",9),
         (try_for_range, ":slot_no", 0, ":times"),
             (store_add,":new_troop",":slot_no","trp_new_troop0"),
             (try_begin),
                 (call_script,"script_party_has_troop","p_main_party",":new_troop"),
                 (eq,reg0,0),
                 (troop_set_slot,"trp_slot1",":slot_no",0),
             (try_end),
         (try_end),
     ]),
    (0,[
        (try_for_range, ":slot_no", 0,9),
            (troop_get_slot, ":root_troop_id", "trp_slot1", ":slot_no"),
            (store_add,":new_troop",":slot_no","trp_new_troop0"),
            (call_script,"script_separate_army",":new_troop",":root_troop_id"),
        (try_end),
        ]),
##########################
...
######### juewei ###### TU #############
	(1,
		[
			(val_add, "$juewei_day", 1),
			(ge, "$juewei_day", 7),
			# (troop_get_slot, ":old_town", "trp_player", slot_troop_town),
			# (troop_get_slot, ":old_troop_is_king", "trp_player", slot_troop_is_king),
			
			(call_script, "script_troop_title_caculate"),
			# (call_script,"script_set_town_lord"),
			
			# (troop_get_slot, ":new_town", "trp_player", slot_troop_town),
			# (str_clear, s46),
			# (try_begin),
				# (neq, ":old_town", ":new_town"),
				# (str_store_party_name, s47, ":new_town"),
				# (str_store_string, s46, "str_move_to_s46"),
			# (try_end),
			(troop_get_slot, ":orginal_title_level", "trp_player", slot_troop_orginal_title_level),
			(troop_get_slot, ":title_level", "trp_player", slot_troop_title_level),
			(try_begin),
				(neg|troop_slot_eq, "trp_player", slot_troop_is_king, -1),
				# (neq, ":old_troop_is_king", -1),
				(gt, ":orginal_title_level", ":title_level"),
				(neq, ":title_level", 9),
				(call_script, "script_get_the_title", "trp_player"),
				(str_store_string, s45, "str_you_have_been_promoted_as_a_juewei"),
				(jump_to_menu, "mnu_title_notification"),
			(else_try),
				(neg|troop_slot_eq, "trp_player", slot_troop_is_king, -1),
				# (neq, ":old_troop_is_king", -1),
				(lt, ":orginal_title_level", ":title_level"),
				(neq, ":title_level", 9),
				(call_script, "script_get_the_title", "trp_player"),
				(str_store_string, s45, "str_you_be_demoted_as_a_juewei"),
				(jump_to_menu, "mnu_title_notification"),
			(else_try),
				(troop_slot_eq, "trp_player", slot_troop_is_king, -1),
				(gt, ":orginal_title_level", ":title_level"),
				(neq, ":title_level", 80),
				(call_script, "script_get_the_title", "trp_player"),
				(str_store_string, s45, "str_you_have_been_promoted_as_a"),
				(jump_to_menu, "mnu_title_notification"),
			(else_try),
				(troop_slot_eq, "trp_player", slot_troop_is_king, -1),
				(lt, ":orginal_title_level", ":title_level"),
				(neq, ":title_level", 80),
				(call_script, "script_get_the_title", "trp_player"),
				(str_store_string, s45, "str_you_be_demoted_as_a"),
				(jump_to_menu, "mnu_title_notification"),
			(try_end),  
			(assign, "$juewei_day", 0),	
        ]),
...
############### TU ###### suijishijian #### 7.9 ###
	(23,
		[
		(store_random_in_range, ":random_num", 0, 36),
		(try_begin),
			(eq, ":random_num", 0),
			(store_random_in_range, ":random_incident", 0, 5),
			(try_begin),
				(eq, ":random_incident", 4),
				(assign, ":random_incident", 0),
				(party_get_num_companions, ":companions_num", "p_main_party"),
				(gt, ":companions_num", 30),
				(start_map_conversation, "trp_huatuo", -1),
			(else_try),
				(eq, ":random_incident", 3),
				(assign, ":random_incident", 0),
				(store_troop_gold, ":player_gold", "trp_player"),
				(ge, ":player_gold", 1000),
				(start_map_conversation, "trp_miheng", -1),
			(else_try),
				(eq, ":random_incident", 2),
				(assign, ":random_incident", 0),
				(eq, "$save_merchant", 1),
				(start_map_conversation, "trp_relative_of_merchant", -1),
			(else_try),
				(eq, ":random_incident", 1),
				(assign, ":random_incident", 0),
				(store_time_of_day, ":cur_hour"),
				(this_or_next|le, ":cur_hour", 5),
				(gt, ":cur_hour", 21),	
				(jump_to_menu, "mnu_suijishijian_cike"),
			(else_try),
				(eq, ":random_incident", 0),
				(jump_to_menu, "mnu_suijishijian_qiangdao"),
			(try_end),		
		(try_end),	
		]),
################################
      ...
      (try_for_range, ":troop_no", heroes_begin, heroes_end),
        ...
        (try_for_range_backwards, ":center_cur_stack", 0, ":center_stacks"),
          ...
          (try_for_range_backwards, ":party_cur_stack", 0, ":party_stacks"),
            ...
            # le 1/3 high-class troops in town/castle, ge 2/3 high-class troops in lord's party 
...
#############
	(24, 
	 [
		(try_for_range, ":troop_no", heroes_begin, heroes_end),
			(troop_slot_eq, ":troop_no", slot_troop_occupation, slto_kingdom_hero),
			(troop_get_slot, ":num", ":troop_no", slot_troop_jinengdian),
			(val_add, ":num", 1),
			(le, ":num", 10),
			(troop_set_slot, ":troop_no", slot_troop_jinengdian, ":num"),
		(try_end),
			(assign, ":troop_no", "trp_player"),
			(troop_slot_eq, ":troop_no", slot_troop_occupation, slto_kingdom_hero),
			(troop_get_slot, ":num", ":troop_no", slot_troop_jinengdian),
	...
	(0, 
	 [
		(try_for_range, ":troop_no", heroes_begin, heroes_end),
			(troop_slot_eq, ":troop_no", slot_troop_occupation, slto_kingdom_hero),
			(troop_get_slot, ":jineng", ":troop_no", slot_troop_jineng),
			(troop_get_slot, ":jinengdian", ":troop_no", slot_troop_jinengdian),
			(try_begin),
				(eq, ":jineng", meirenji),
				(troop_get_slot, ":flag", ":troop_no", slot_troop_jineng_flag),
				# (troop_get_slot, ":dianshu", ":troop_no", slot_troop_jinengdian),
				(troop_get_slot, ":party_no", ":troop_no", slot_troop_jineng_object),
				(try_begin),
					(eq, ":flag", 1),
					(gt, ":party_no", 0),
					(troop_get_slot, ":time", ":troop_no", slot_troop_jineng_time_begin),
					(val_add, ":time", 24),
					(store_current_hours, ":current_hours"),
					(try_begin),
						(gt, ":current_hours", ":time"),
						(troop_set_slot, ":troop_no", slot_troop_jineng_flag, 0),
					(try_end),
					(le, ":current_hours", ":time"),
					
					(party_is_active, ":party_no"),

					(party_set_ai_initiative, ":party_no", 0),
					# (party_set_slot, ":party_no", slot_party_type, spt_kingdom_caravan),
				...
				(else_try),
					...
					(try_end),
					(le, ":current_hours", ":time"),
					(val_sub, ":time", ":current_hours"),
					(rest_for_hours, ":time", 5, 1), #rest while not attackable
				(try_end),
			(else_try),
				(eq, ":jineng", kongchengji),
				(ge, ":jinengdian", 7),
				(store_troop_faction, ":troop_faction", ":troop_no"),
				(try_for_range, ":party_no", walled_centers_begin, walled_centers_end),
					(assign, ":continue", 0),
					(party_get_slot, ":flag", ":party_no", slot_party_jineng_kongchengji_flag),
					(eq, ":flag", 0),
					(try_begin),
						(party_get_slot, ":party_state", ":party_no", slot_village_state),
						(eq, ":party_state", svs_under_siege),
						(store_faction_of_party, ":party_faction", ":party_no"),
						(eq, ":troop_faction", ":party_faction"),
						(assign, ":continue", 1),
					(else_try),
						(eq, "$g_player_besiege_town", ":party_no"),
					...
					(store_current_hours, ":current_hours"),
					(party_set_slot, ":party_no", slot_party_jineng_kongchengji_flag, 1),
					(party_set_slot, ":party_no", slot_party_jineng_kongchengji_begin, ":current_hours"),
					
					(str_store_troop_name, s1, ":troop_no"),
					(str_store_string, s2, "str_kongchengji"),
					(str_store_party_name, s3, ":party_no"),
					(display_message, "str_jineng_s1_use_s2_to_s3", 0xFFFFAAAA),
				(try_end),
			(try_end),
		(try_end),
			(assign, ":troop_no", "trp_player"),
			...
			(try_begin),
				(gt, ":current_hours", ":time"),
				(troop_set_slot, ":troop_no", slot_troop_jineng_flag, 0),
				(str_store_string, s1, "str_meirenji"),
				(display_message, "str_you_s1_finish", 0xFFFFAAAA),
			(try_end),
			(le, ":current_hours", ":time"),
...
############ jineng_xiaobing #######################
    (1,[
        ...
        (set_camera_follow_party, "$teacher_party"),
        (party_relocate_near_party, "p_main_party", "$teacher_party", 1), 
  ...
  #  WEEKLY PAY AND CHECKS FOR UPGRADE
    ...
    (24, [
        ...
        (troop_get_slot, ":day", "trp_player", slot_troop_jineng_have_during),
		(val_add, ":day", 1),
        (troop_set_slot, "trp_player", slot_troop_jineng_have_during, ":day"),
		(ge, ":day", 20),
		(lt, ":day", 21),
		(troop_get_slot, ":jineng", "$teacher_lord", slot_troop_jineng),
		(troop_set_slot, "trp_player", slot_troop_jineng, ":jineng"),
		(jump_to_menu, "mnu_learn_success"),
    ]),
###################################
]
